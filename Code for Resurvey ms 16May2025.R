...
## data available in this Github repository (ameliawolfUT/Wolf-et-al-Coyote_RidgeResurvey)

#read in data
#CRcover=read.csv("CRresurveyforR.csv", sep=""), stringsAsFactors=F)
#polycult=read.csv("polycultforR24Feb2025.csv", sep=""), stringsAsFactors=F)
#polycult$block.chr<-as.character(polycult$block)
#polycultgraph=read.csv("polycultforgraphs.csv", sep=""), stringsAsFactors=F)
#CRbiomass=read.csv("CRresurveybiomassR30Jan2024.csv", sep=""), stringsAsFactors=F)
#CRbiomass$block<-as.character(CRbiomass$block)

##TAKE THIS OUT BELOW BEFORE POSTING - READ IN LINES ARE ABOVE##
CRcover=read.csv(file=paste("//Users//acacia//Dropbox//CR resurvey 2017//CRresurveyforR.csv", sep=""), stringsAsFactors=F)
names(CRcover)
str(CRcover)
polycult=read.csv(file=paste("//Users//acacia//Dropbox//CR resurvey 2017//polycultforR24Feb2025.csv", sep=""), stringsAsFactors=F) #includes Nfixer cover
polycult$block.chr<-as.character(polycult$block)
polycultgraph=read.csv(file=paste("////Users//acacia//Dropbox//CR resurvey 2017//polycultforgraphs.csv", sep=""), stringsAsFactors=F)
CRbiomass=read.csv(file=paste("//Users//acacia//Dropbox//CR resurvey 2017//CRresurveybiomassR30Jan2024.csv", sep=""), stringsAsFactors=F)
CRbiomass$block<-as.character(CRbiomass$block)
str(CRbiomass)
...
# Model with Nfixer cover needs to be fit using Negative Binomial distribution
m7_negbin <- glmer.nb((scaled.Nfixer.cover+1) ~ spnum * treat * depth + (1 | block), 
                       data = polycult,
...
CRcoverforbox <- CRcover
CRcoverforbox$depth <- factor(CRcoverforbox$depth, c("Shallow", "Medium", "Deep"))
polycultforbox <- polycult
polycultforbox$depth <- factor(polycultforbox$depth, c("Shallow", "Medium", "Deep"))
...
PCord <- subset(polycult, treat=="Order")
PCran <- subset(polycult, treat=="Random")
PCordsha <- subset(PCord, depth=="Shallow")
PCordmed <- subset(PCord, depth=="Medium")
PCorddee <- subset(PCord, depth=="Deep")
PCransha <- subset(PCran, depth=="Shallow")
PCranmed <- subset(PCran, depth=="Medium")
PCrandee <- subset(PCran, depth=="Deep")
PCsha <- subset(polycult, depth=="Shallow")
PCmed <- subset(polycult, depth=="Medium")
PCdee <- subset(polycult, depth=="Deep")
...
mord <- lmer(sqrt(native.richness+1) ~ spnum*depth+(1|block), data=PCord)
anova(mord)
mran <- lmer(sqrt(native.richness+1) ~ spnum*depth+(1|block), data=PCran)
anova(mran)
mran <- lm(sqrt(native.richness+1) ~ spnum*depth, data=PCran)
anova(mran)
...
anova(lm(CRbioOrdSha$invprod~CRbioOrdSha$spnum))
...
anova(lm(CRbioOrdDee$invprod~CRbioOrdDee$spnum))
...
plot(CRbioRanDee$spnum, CRbioRanDee$invprod, main="Random species loss", xlab="Experimental phase target richnesss", ylab=expression("Post-recovery Invasive Productivity (g m"^-2*" yr"^-1*")"), ylim=c(150,900), pch=19)abline(lm(CRbioRanDee$invprod~CRbioRanDee$spnum), lty=2)
...
plot(jitter(PCran$spnum, factor = .4), PCran$scaled.Nfixer.cover, main="Random species loss", xlab="Experimental phase target diversity", ylab="Post-drought recovery N fixer cover (%)", ylim=c(0, 18), pch=19)
abline(lm(PCran$scaled.Nfixer.cover~PCran$spnum), lty = 2)

plot(jitter(PCord$spnum, factor = .4), PCord$scaled.Nfixer.cover, main="Realistic species loss", xlab="Experimental phase target diversity", ylab="Post-drought recovery N fixer cover (%)", ylim=c(0, 18), pch=19)
abline(lm(PCord$scaled.Nfixer.cover~PCord$spnum), lty = 2)
